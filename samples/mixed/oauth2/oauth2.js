...
OAuth2.prototype.getGCAccessAndRefreshTokens = function (
  ...
  const adapter = this.adapter ? this.adapter : this.OAuth2.adapter();
  ...
  xhr.addEventListener("readystatechange", function (event) {
    if (xhr.readyState == 4) {
      if (xhr.status == 201) {
        ...
        callback(adapter.parseAccessGCToken(xhr.responseText));
      }
    }
  });

  const method = adapter.accessTokenMethod();
  const items = {
    source: "hubspot",
    crmCode: authorizationCode,
  };

  // open request
  xhr.open(method, adapter.accessTokenGCURL(), true);
...
OAuth2.prototype.getAccessAndRefreshTokens = function (
  ...
  if (that.adapterName === "hubspot") {
    that.getGCAccessAndRefreshTokens(authorizationCode, callback);
    return;
  }
